...
export const ExpenseComments: React.FC<ExpenseCommentsProps> = ({ expenseId }) => {
  ...
  useEffect(() => {
    ...
    const handleNewComment = (payload: { expenseId: string; comment: Comment }) => {
    ...
    socket.on('expense_comment', handleNewComment);
    return () => {
      socket.off('expense_comment', handleNewComment);
    };
  }, [isExpanded, expenseId]);
                ...
                comments.map(c => (
                  <div key={c.id} className="flex gap-2.5">
                    <div className="w-6 h-6 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center text-[10px] font-bold shrink-0">
                      {c.avatar_url ? (
                        <img src={`http://localhost:3000${c.avatar_url}`} alt={c.user_name} className="w-full h-full rounded-full object-cover" />
                      ) : (
                    ...
                    <div className="flex-1 bg-white/5 border border-white/5 rounded-lg rounded-tl-none p-2 relative group">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] font-bold text-zinc-300">{c.user_id === currentUser?.id ? 'You' : c.user_name}</span>
                        <span className="text-[9px] text-zinc-500">{new Date(c.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
