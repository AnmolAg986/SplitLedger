...
import { Mail, LockKeyhole, ArrowRight, Activity, Eye, EyeOff, Key, X, ShieldCheck } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (Array.isArray(e.response?.data?.error)) {
        ...
        setError((e.response?.data?.error as string) || 'Invalid email or password.');
      }
    } finally {
      setLoading(false);
    }
  ...
  if (requires2FA) {
    return (
      <div className="relative h-screen w-full flex items-center justify-center p-4 bg-[#050505] font-sans overflow-y-auto">
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-cyan-500/5 rounded-full blur-[120px] pointer-events-none" />
        <div className="relative z-10 w-full max-w-[440px] bg-[#0c0c0e] border border-white/[0.08] rounded-[32px] shadow-[0_0_80px_rgba(0,0,0,0.8)] p-7 sm:p-10 mx-auto">
          <div className="flex flex-col items-center text-center mb-8">
            <div className="h-14 w-14 bg-cyan-500/10 border border-cyan-500/20 rounded-2xl flex items-center justify-center mb-5">
              <ShieldCheck className="h-7 w-7 text-cyan-400" strokeWidth={2} />
            </div>
            <h2 className="font-display text-2xl font-bold tracking-tight mb-2 text-white">Two-Factor Auth</h2>
            <p className="text-zinc-400 text-[14px]">Enter the 6-digit code from your authenticator app, or use a recovery code.</p>
          </div>

          {error && (
            <div className="w-full mb-6 p-4 text-[13px] font-semibold rounded-xl flex items-center gap-3 text-rose-300 bg-rose-500/10 border border-rose-500/20 animate-in fade-in">
          ...
          <form onSubmit={handle2FASubmit} className="space-y-5">
            <div className="space-y-2">
              <label className="block text-[12px] font-bold text-zinc-400 uppercase tracking-[0.1em] ml-1">
                Authentication Code
              </label>
              <input
                id="totp-code"
                ...
                maxLength={23}
                className="w-full px-5 py-4 bg-[#050505] border border-white/5 rounded-xl focus:border-cyan-500/50 focus:ring-4 focus:ring-cyan-500/5 outline-none transition-all text-[22px] font-bold text-white placeholder-zinc-700 text-center tracking-[0.4em]"
                placeholder="000000"
                value={totpCode}
            ...
            <button
              ...
              type="submit"
              disabled={loading}
              className="w-full flex justify-center items-center py-4 px-4 text-[15px] font-bold rounded-xl text-black bg-cyan-400 hover:bg-cyan-300 focus:outline-none transition-all duration-200 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
            >
          ...
          <button
            onClick={() => { setRequires2FA(false); setError(''); setTotpCode(''); }}
            className="mt-5 w-full text-center text-[13px] text-zinc-500 hover:text-zinc-300 transition-colors"
          >
            ← Back to login
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-start justify-center sm:justify-end p-4 sm:p-6 pointer-events-none">
          <div className="pointer-events-auto w-full max-w-[360px] bg-[#141416] border border-white/[0.12] rounded-2xl shadow-[0_8px_60px_rgba(0,0,0,0.9)] p-5 animate-in slide-in-from-bottom-4 sm:slide-in-from-right-4 duration-300">
            <div className="flex items-start gap-3">
              ...
              <button onClick={handleDismiss} className="shrink-0 text-zinc-600 hover:text-zinc-300 transition-colors mt-0.5" aria-label="Dismiss">
                <X className="h-4 w-4" strokeWidth={2} />
              </button>
            </div>
            <div className="flex gap-2 mt-4">
              <button onClick={handleSave} className="flex-1 py-2 px-3 text-[13px] font-bold bg-white text-black rounded-lg hover:bg-zinc-200 transition-colors active:scale-[0.97]">Save</button>
              <button onClick={handleDontAsk} className="flex-1 py-2 px-3 text-[13px] font-bold bg-white/10 text-zinc-300 rounded-lg hover:bg-white/15 transition-colors active:scale-[0.97] border border-white/10">Don't ask again</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="relative z-10 w-full max-w-[440px] bg-[#0c0c0e] border border-white/[0.08] rounded-[32px] shadow-[0_0_80px_rgba(0,0,0,0.8)] p-7 sm:p-10 mx-auto">
        <div className="flex flex-col">
          <div className="flex flex-col items-center text-center mb-10">
            ...
            <h2 className="font-display text-3xl font-bold tracking-tight mb-2 text-white">SplitLedger</h2>
            <p className="text-zinc-500 text-[14px] font-medium opacity-80">Access your secure account.</p>
          </div>
          ...
          <form className="w-full space-y-6" noValidate onSubmit={handleSubmit}>
            <div className="space-y-2">
              <label htmlFor="identifier" className="block text-[12px] font-bold text-zinc-400 uppercase tracking-[0.1em] ml-1">Email or Phone</label>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center ml-1">
                <label htmlFor="password" className="text-[12px] font-bold text-zinc-400 uppercase tracking-[0.1em]">Password</label>
                <Link to="/forgot-password" className="text-[12px] font-bold text-indigo-400 hover:text-indigo-300 transition-colors">Forgot?</Link>
              </div>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-zinc-500 hover:text-zinc-300 transition-colors" tabIndex={-1} aria-label={showPassword ? 'Hide password' : 'Show password'}>
          ...
          <div className="mt-10 pt-8 border-t border-white/5 text-center">
            <p className="text-[14px] font-medium text-zinc-500">
              ...
              <Link to="/signup" className="text-white hover:text-indigo-400 font-bold transition-colors ml-1">Create one</Link>
            </p>
          </div>
        </div>
      </div>
