            ...
            {isEditing ? (
              <div className="space-y-4">
                <div className="relative">
                  ...
                  >
                     {editForm.avatarUrl ? (
                       <LazyImage src={`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}${editForm.avatarUrl}`} alt="Group" className="w-full h-full object-cover" />
                     ) : (
              ...
              <>
                <div 
                  ...
                  onClick={() => detail.avatar_url && window.open(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}${detail.avatar_url}`, '_blank')}
                >
                  {detail.avatar_url ? (
                     <LazyImage src={`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}${detail.avatar_url}`} alt="Group" className="w-full h-full object-cover" />
                   ) : (
                ...
                {searchResults.map(f => (
                  ...
                  >
                    <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-bold text-white overflow-hidden">
                      {f.avatar_url ? (
                        <LazyImage src={`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}${f.avatar_url}`} alt={f.display_name} className="w-full h-full object-cover" />
                      ) : (
                ...
                return (
                  <div key={m.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/[0.03] transition-colors group">
                    <div className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-xs font-bold text-white shrink-0 overflow-hidden">
                      {m.avatar_url ? (
                        <LazyImage src={`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}${m.avatar_url}`} alt={m.display_name} className="w-full h-full object-cover" />
                      ) : (
