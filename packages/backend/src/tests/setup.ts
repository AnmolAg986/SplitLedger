...
afterAll(async () => {
  // Do NOT call pool.end() here. With singleFork/fileParallelism:false,
  // all test files share one process and one pool. Calling pool.end()
  // after the first file closes the connection for all subsequent files.
  // The pool is cleaned up automatically when the process exits.
});
